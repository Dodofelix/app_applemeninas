...
import { Send, Download, Eye, Loader2, Trash2, Search, Copy } from "lucide-react";
      ...
      <Card className="shadow-card">
        ...
        <CardContent className="p-0 overflow-hidden">
              ...
              {isLoading ? (
                ...
                filteredOrders.map((order) => (
                  <TableRow key={order.id} className="hover:bg-muted/50 transition-colors">
                    <TableCell className="text-sm">{order.cliente_nome}</TableCell>
                    <TableCell className="text-sm">{order.produto}</TableCell>
                    <TableCell className="text-sm">{formatCurrency(order.valor)}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">{order.forma_pagamento}</TableCell>
                    <TableCell>
                      <Select
                    ...
                    <TableCell className="text-sm text-muted-foreground">{formatDate(order.data)}</TableCell>
                    <TableCell>
                      <div className="flex flex-wrap gap-1 items-center">
                        ...
                        <Button
                          size="icon"
                          variant="ghost"
                          title="Copiar resumo de pagamento"
                          onClick={() => handleCopiarResumo(order)}
                        >
                        ...
                        <Button
                          size="icon"
                          variant="ghost"
                          title="Copiar formulário NF"
                          onClick={() => handleCopiarFormularioNF(order)}
                        >
                          <Copy className="h-3.5 w-3.5" />
                        </Button>
                        <Button
                        ...
                        {order.status === "Contrato gerado" && (
                          <Button size="icon" variant="ghost" onClick={() => toast.info("Enviar para assinatura (em breve)")} title="Enviar assinatura">
          ...
          <div className="md:hidden space-y-4 p-4 pt-0">
            ...
            {!isLoading && filteredOrders.length === 0 && searchQuery.trim() && (
              <p className="text-center text-muted-foreground py-8">Nenhum pedido encontrado para &quot;{searchQuery}&quot;.</p>
            )}
            {!isLoading && filteredOrders.length > 0 && filteredOrders.map((order) => (
              <Card key={order.id} className="border bg-card p-4 shadow-sm">
                <div className="space-y-3 text-sm">
                  <p><span className="text-muted-foreground">Cliente:</span> <span className="font-medium">{order.cliente_nome}</span></p>
                  <p><span className="text-muted-foreground">Produto:</span> {order.produto}</p>
                  <p><span className="text-muted-foreground">Valor:</span> {formatCurrency(order.valor)}</p>
                  <p><span className="text-muted-foreground">Pagamento:</span> {order.forma_pagamento}</p>
                  ...
                  <div className="flex flex-wrap gap-2 pt-2 border-t">
                    ...
                    <Button size="sm" variant="outline" onClick={() => handleCopiarFormularioNF(order)}>
                      <Copy className="h-4 w-4" />
                      <span className="ml-1.5">Copiar NF</span>
                    </Button>
                    <Button size="sm" variant="outline" className="text-destructive hover:text-destructive" onClick={() => setOrderToDelete(order)} disabled={deleteOrder.isPending}>
