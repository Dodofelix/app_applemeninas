...
import { LayoutList, Plus, Pencil, Trash2, GripVertical } from "lucide-react";
...
import type { DemandCard, DemandColumn } from "@/lib/firestore";
    ...
    >
      <CardContent className="p-3 flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {card.dueDate && <p className="text-xs text-muted-foreground mt-0.5">Entrega: {formatDueDate(card.dueDate)}</p>}
        </div>
    ...
    >
      ...
      <SortableContext items={cardIds} strategy={verticalListSortingStrategy}>
        <div className="flex-1 p-2 space-y-2 overflow-y-auto min-h-0">
          {cards.map((card) => (
            <DemandCardItem key={card.id} card={card} onEdit={onEditCard} onDelete={onDeleteCard} />
          ))}
        </div>
      </SortableContext>
...
export default function Demandas2() {
  ...
  const [newCardDescricao, setNewCardDescricao] = useState("");
  ...
  const [editCardDescricao, setEditCardDescricao] = useState("");
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      const activeCardNow = cards.find((c) => c.id === String(active.id));
      if (!activeCardNow) return;

      const overId = String(over.id);
      const isOverColumn = columns.some((col) => col.id === overId);
      const overCard = cards.find((c) => c.id === overId);

      const sourceColumnId = activeCardNow.columnId;
      ...
      if (isOverColumn) {
        ...
        const idx = inTarget.findIndex((c) => c.id === overCard.id);
        insertIndex = idx >= 0 ? idx : inTarget.length;
      } else {
        return;
      }

      try {
        // Reordenar origem quando muda de coluna
        if (sourceColumnId !== targetColumnId) {
          const inSource = cards
            .filter((c) => c.columnId === sourceColumnId && c.id !== activeCardNow.id)
            .sort((a, b) => a.order - b.order);
          for (let i = 0; i < inSource.length; i++) {
            await updateCard.mutateAsync({ id: inSource[i].id, data: { order: i } });
          }
        }

        const inTargetWithoutActive = cards
          .filter((c) => c.columnId === targetColumnId && c.id !== activeCardNow.id)
          .sort((a, b) => a.order - b.order);

        const finalTarget = [...inTargetWithoutActive];
        const safeInsertIndex = Math.max(0, Math.min(insertIndex, finalTarget.length));
        finalTarget.splice(safeInsertIndex, 0, { ...activeCardNow, columnId: targetColumnId });

        for (let i = 0; i < finalTarget.length; i++) {
          await updateCard.mutateAsync({
            id: finalTarget[i].id,
            data: { columnId: targetColumnId, order: i },
          });
        }
  ...
  const handleAddCard = async () => {
    ...
    const order = cards.filter((c) => c.columnId === addCardColumnId).length;
    try {
  ...
  const openEditCard = (card: DemandCard) => {
    ...
    setEditCardDescricao(card.descricao || "");
    ...
    <div className="space-y-4">
            ...
            <Card className="w-[260px] cursor-grabbing shadow-lg ring-2 ring-primary/20 opacity-95">
              <CardContent className="p-3 space-y-1">
                ...
                {activeCard.descricao && (
                  <p className="text-xs text-muted-foreground line-clamp-2 break-words">{activeCard.descricao}</p>
                )}
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Crie a primeira coluna para começar a organizar as demandas.
            </p>
      ...
      <Dialog open={addCardColumnId !== null} onOpenChange={(open) => !open && setAddCardColumnId(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAddCardColumnId(null)}>
              Cancelar
            </Button>
